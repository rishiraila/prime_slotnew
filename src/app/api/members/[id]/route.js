...
export async function POST(req, ctx) {
  try {
    ...
    const meetingId = ref.key;

    // 🧩 Add calendar index entries for both members
    const indexA = {
      eventId,
      meetingId,
      scheduledAt: when,
    ...
    const indexB = {
      ...
      durationMin: input.durationMin,
      status: input.status,
      otherPartyId: input.aId,
      topic: input.topic,
    };

    await Promise.all([
      rtdb.ref(`/memberMeetings/${input.aId}/${eventId}/${meetingId}`).set(indexA),
      rtdb.ref(`/memberMeetings/${input.bId}/${eventId}/${meetingId}`).set(indexB),
    ]);

    return addCORS(NextResponse.json({ id: meetingId }, { status: 201 }));
  } catch (e) {
    return addCORS(NextResponse.json({ error: e.message || 'Server error' }, { status: e.status || 500 }));
  }
}
