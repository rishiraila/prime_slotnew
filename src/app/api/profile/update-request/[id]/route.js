...
export async function GET(req, { params }) {
  try {
    ...
    const snap = await rtdb.ref(`/profileUpdateRequests/${id}`).once('value');
    if (!snap.exists()) return addCORS(NextResponse.json({ ok:false, message:'not found' }, { status:404 }));

    return addCORS(NextResponse.json({ ok:true, item: snap.val() }, { status:200 }));
  } catch (err) {
    console.error('get update request error', err);
