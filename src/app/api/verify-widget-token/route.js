  ...
  for(const [id, rec] of Object.entries(members)){
    ...
    for(const p of candidates){
      ...
      if(typeof p === 'string' || typeof p === 'number') candidate = String(p);
        ...
        const keys = ['Value','value','phone','Phone','mobile','Mobile','number','contact','phoneRaw'];
        for(const k of keys) if(k in p && p[k]!=null){ candidate = String(p[k]); break; }
        if(!candidate){
...
export async function POST(request){
  try{
    ...
    if(!accessToken) return addCORS(NextResponse.json({ success:false, message:'Missing accessToken' }, { status:400 }));

    const MSG91_AUTHKEY = process.env.MSG91_AUTHKEY || body?.MSG91_AUTHKEY;
    if(!MSG91_AUTHKEY) return addCORS(NextResponse.json({ success:false, message:'MSG91_AUTHKEY missing' }, { status:500 }));
    ...
    if(!resp.ok){
      return addCORS(NextResponse.json({ success:false, message:'MSG91 verify failed', status: resp.status, raw: data }, { status:400 }));
    }

    const verified = (data?.type === 'success' || data?.status === 'success' || data?.success === true);
    if(!verified) return addCORS(NextResponse.json({ success:false, message: data?.message || 'Invalid access token', raw: data }, { status:400 }));

    const phoneDigits = extractPhoneFromMsg91Response(data) || digitsOnly(body?.phone) || null;
    if(!phoneDigits) return addCORS(NextResponse.json({ success:false, message:'Phone not found in token response', raw: data }, { status:400 }));

    const matched = await findMemberByPhoneDigits(phoneDigits);
    if(!matched) return addCORS(NextResponse.json({ success:false, message:'Member not found', phoneDigits }, { status:404 }));
    ...
    const token = jwt.sign({ sub: matched.id, provider: 'msg91-widget' }, JWT_SECRET, { expiresIn: maxAge });
    const resOut = NextResponse.json({ ...responseBody, token }, { status: 200 });
    resOut.cookies.set('session', token, { httpOnly:true, path:'/', maxAge, secure: process.env.NODE_ENV === 'production', sameSite:'lax' });
